...
contract Ethernaut is Ownable {
  ...
  function createLevelInstance(Level _level) public payable returns (address) {
    ...
    return instance; // PATCH: return data because it's not possible to read emitted events via Solidity
  }
  ...
  function submitLevelInstance(address payable _instance) public returns (bool) {
    ...
    if(data.level.validateInstance(_instance, msg.sender)) {
      ...
      return true;  // PATCH: return data because it's not possible to read emitted events
    }

    return false;  // PATCH: return data because it's not possible to read emitted events
  }
}